...
- OpenAI calls are lazy and environment-configured.
- Deterministic fallbacks still power local demos.
- Unit tests mock the OpenAI client and avoid network calls.

Still needed:

- Create the GitHub PR once the repository has a remote and an initial baseline.
